...
//        );
    ...
    public String handleResultClient() throws SQLException {
        int timeClient1 = 0;
        int timeClient2 = 0;
        int totalValueClient1 = 0;
        int totalValueClient2 = 0;

        if (resultClient1 != null) {
            String[] splitted1 = resultClient1.split(";");
            timeClient1 = Integer.parseInt(splitted1[5]); // Assuming this index has the time value
            totalValueClient1 = extractTotalValue(splitted1[splitted1.length - 1]);
        }
        if (resultClient2 != null) {
            String[] splitted2 = resultClient2.split(";");
            timeClient2 = Integer.parseInt(splitted2[5]); // Assuming this index has the time value
            totalValueClient2 = extractTotalValue(splitted2[splitted2.length - 1]);
        }
        ...
        if (resultClient1 == null && resultClient2 == null) {
            ...
            if (totalValueClient1 > totalValueClient2) {
                client1Win(timeClient1);
                return client1.getLoginUser();
            } else if (totalValueClient1 < totalValueClient2) {
    ...
    private int extractTotalValue(String totalValueString) {
        try {
            return Integer.parseInt(totalValueString);
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
